...
class Evaluator():
    ...
    def compute_hungarian_cost(self, pred, targets):
        ...
        padding = torch.zeros(
            pred.shape[0], pred.shape[1], clusters, device=pred.device)
        random_clusters = torch.randint(
            0, clusters, (pred.shape[0], pred.shape[1], 1), device=pred.device).long()
        random_clusters = torch.nn.functional.one_hot(
            random_clusters.squeeze(), num_classes=clusters).unsqueeze(dim=0).float()
       ...
       # print(pred_resized.shape,targets.shape)
        ...
        cost_matrix = torch.zeros(
            clusters, self.unkown_classes, device=pred.device)
        ...
        for cluster in range(clusters):
            for label in range(self.unkown_classes):
                ...
                cost_matrix[cluster, label] = (
                    (pred_copy == targets_indices) & unknown_mask).sum(dim=-1).item()
