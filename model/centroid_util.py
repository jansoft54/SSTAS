...
def update_centroids(config: TrainerConfig, model, model_out, target_truth, known_mask, unknown_mask=None):
    ...
    with torch.no_grad():
        ...
        if known_mask.any():
            ...
            for c in range(config.known_classes):
                class_mask = (lbls_k == c)
                if class_mask.any():
        ...
        if model.class_centers.shape[0] > config.known_classes and unknown_mask is not None and unknown_mask.any():
            ...
            pseudo_logits = unk_logits[:, config.known_classes:] # [N_unk_frames, 10]
            winning_slots = torch.argmax(pseudo_logits, dim=-1) # Werte von 0 bis 9
         ...
         #   raise Exception(len(pseudo_logits[0]))
            ...
            for s in range(num_pseudo):
                ...
                if slot_mask.any():
                    ...
                    target_idx = config.known_classes + s
